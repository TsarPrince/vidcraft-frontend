          ...
          <tbody>
            {
              bucketItems?.filter((item) => {
                // filter out the files not having id
                // folders / directories will not have id
                return item.id
              }).map((item, index) => (
                <tr key={index} className="border-b">
                  <td className="py-2 pr-12 pl-4">{index + 1}</td>
                  <td className="py-2 pr-12 whitespace-nowrap">{item.id ?? '-'}</td>
                  <td className="py-2 pr-12 whitespace-nowrap">
                    <a
                      ...
                      target='_blank'
                      href={encodeURI(`${NEXT_PUBLIC_SUPABASE_VIDEO_URL}/${item.name}`)}
                    >
                  ...
                  <td className="py-2 pr-12 whitespace-nowrap">{item.updated_at ?? '-'}</td>
                  <td className="py-2 pr-12 whitespace-nowrap">{item.created_at ?? '-'}</td>
                  <td className="py-2 pr-12 whitespace-nowrap">{(item.metadata?.size / 1024 / 1024).toFixed(2) + ' MB' ?? '-'}</td>
                  <td className="py-2 pr-12 whitespace-nowrap">{item.metadata?.mimetype ?? '-'}</td>
                </tr>
              ))
            }
          </tbody>
